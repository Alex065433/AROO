...
router.post("/user-action", async (req, res) => {
  ...
  console.log('Sync: Received request to user-action');
  ...
  if (!uid || amount == null) {
    console.error('V1: Missing parameters for add-funds:', { uid, amount });
  ...
  if (!token) {
    console.error('V1: Unauthorized attempt to add-funds (missing token)');
  ...
  try {
    ...
    console.error('Error in u-p-b API:', error);
    res.status(500).json({ 
...
router.post("/admin-query", async (req, res) => {
  ...
  const token = customToken || (authHeader?.startsWith('Bearer ') ? authHeader.split(' ')[1] : null);
...
router.post("/admin-package", async (req, res) => {
  ...
  if (!uid || packageAmount === undefined || packageAmount === null) {
    console.error('Admin: Missing required fields for p-a:', { uid, packageAmount, body: req.body });
    return res.status(400).json({ 
  ...
  try {
    ...
    if (isNaN(numericAmount)) {
      console.error('Admin: Invalid package amount format for p-a:', { packageAmount });
    ...
    console.error('Error in p-a API:', error);
    res.status(500).json({ 
