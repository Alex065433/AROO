-- Consolidated Supabase Fix for Type Mismatches and Wallet Updates
-- This script fixes the "column 'uid' is of type uuid but expression is of type text" error
-- and ensures wallets are updated correctly via database triggers.

-- 1. Drop all possible versions of the RPC function to avoid ambiguity
...
-- 2. Create the robust RPC function with explicit UUID casting and JSON return
...
BEGIN
    ...
    -- Insert payment record with explicit UUID casting for p_uid
...
-- 3. Fix update_wallets_on_payment trigger function
...
BEGIN
    ...
    IF (NEW.status = 'finished' OR NEW.status = 'completed') AND (TG_OP = 'INSERT' OR (TG_OP = 'UPDATE' AND (OLD.status IS NULL OR OLD.status NOT IN ('finished', 'completed')))) THEN
        ...
        VALUES (NEW.uid, NEW.amount, NEW.type, COALESCE(NEW.order_description, NEW.type) || ' (' || NEW.type || ')', NOW());
...
-- 6. Sync current wallet_balance column with JSONB master balance for consistency
UPDATE public.profiles 
