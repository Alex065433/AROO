...
import { UserPlus, ShieldCheck, Mail, Phone, Lock, ChevronDown, ArrowRight, RefreshCw, CheckCircle2, Eye, EyeOff, AlertCircle, LogOut, Github } from 'lucide-react';
...
const Register: React.FC<{ onLogin: () => void }> = ({ onLogin }) => {
  ...
  useEffect(() => {
    const fetchSponsor = async () => {
      if (sponsorId && sponsorId.length >= 6) {
  ...
  const handleGithubLogin = async () => {
    setError(null);
    try {
      await supabaseService.loginWithGithub();
      // The redirect will handle the rest
    } catch (err: any) {
            ...
            <div className="md:col-span-2 pt-8">
              ...
              <div className="grid grid-cols-2 gap-4">
                <button 
                  ...
                  className="w-full bg-white/5 hover:bg-white/10 text-white font-black py-5 rounded-2xl border border-white/5 transition-all flex items-center justify-center gap-3 group active:scale-95 text-[10px] uppercase tracking-widest"
                >
